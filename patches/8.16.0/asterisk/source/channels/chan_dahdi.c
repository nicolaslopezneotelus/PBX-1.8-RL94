...
#endif	/* defined(HAVE_PRI) */
{
	...
	if (i->channel == CHAN_PSEUDO) {
		...
		//DELETED: 2012-07-11
		//ADDED: 2012-07-11
		ast_str_set(&chan_name, 0, "%d-%x", i->channel, y);
		//ADDED: 2012-07-11
...
static struct ast_channel *dahdi_new(struct dahdi_pvt *i, int state, int startpbx, int idx, int law, const char *linkedid)
{
	...
	//ADDED: 2012-11-08
	i->subs[idx].needringing = 0;
	i->subs[idx].needbusy = 0;
	i->subs[idx].needcongestion = 0;
	i->subs[idx].needanswer = 0;
	i->subs[idx].needflash = 0;
	i->subs[idx].needhold = 0;
	i->subs[idx].needunhold = 0;
	//ADDED: 2012-11-08
	i->subs[idx].linear = 0;
