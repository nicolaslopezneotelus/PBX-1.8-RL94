	...
	for (;;) {
		...
		if (e) {
			...
			case PRI_EVENT_RINGING:
				...
				//ADDED: 2010-11-26
				if (e->proceeding.progressmask & PRI_PROG_CALL_NOT_E2E_ISDN) {
					struct ast_channel *owner;
					...
					if (owner) {
						char name[256];
						ast_copy_string(name, owner->name, sizeof(name));
						manager_event(EVENT_FLAG_SYSTEM, "PRI",
							"Type: PRI_EVENT_RINGING\r\n"
							"Subtype: PRI_PROG_CALL_NOT_E2E_ISDN\r\n"
							"Channel: %s\r\n",
							name);
						ast_channel_unlock(owner);
					}
			...
			case PRI_EVENT_NOTIFY:
				...
				case PRI_NOTIFY_REMOTE_RETRIEVAL:
					...
					break;
				//ADDED: 2008-05-19
				case PRI_NOTIFY_USER_SUSPENDED:
					{
						struct ast_channel *owner;
						...
						if (owner) {
							...
							ast_copy_string(name, owner->name, sizeof(name));
							manager_event(EVENT_FLAG_SYSTEM, "PRI",
								"Type: PRI_EVENT_NOTIFY\r\n"
								"Subtype: PRI_NOTIFY_USER_SUSPENDED\r\n"
								"Channel: %s\r\n",
								name);
							ast_channel_unlock(owner);
						}
