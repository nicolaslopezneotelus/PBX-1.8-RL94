...
static void senddialevent(struct ast_channel *src, struct ast_channel *dst, const char *dialstring)
{
	...
	ast_manager_event_multichan(EVENT_FLAG_CALL, "Dial", 2, chans,
		...
		//ADDED: 2012-03-12
		pbx_builtin_getvar_helper(src, "FEATURECODE"), pbx_builtin_getvar_helper(dst, "FEATURECODE"),
		//ADDED: 2012-03-12
...
static int dial_exec_full(struct ast_channel *chan, const char *data, struct ast_flags64 *peerflags, int *continue_exec)
{
	...
	while ((cur = strsep(&rest, "&")) ) {
		...
		//ADDED: 2012-07-27
		const char *temp_id = pbx_builtin_getvar_helper(tc, "TEMP_ID");
		if (temp_id) {
			manager_event(EVENT_FLAG_CALL, "NewTempId",
			"Channel: %s\r\n"
			"Uniqueid: %s\r\n"
			"TempId: %s\r\n",
			tc->name, tc->uniqueid, temp_id);
		}
		//ADDED: 2012-07-27
