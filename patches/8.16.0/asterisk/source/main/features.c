...
//ADDED: 2012-02-16
static int check_builtin_feature(const char *code);
//ADDED: 2012-02-16
...
static int builtin_blindtransfer(struct ast_channel *chan, struct ast_channel *peer, struct ast_bridge_config *config, const char *code, int sense, void *data)
{
	...
	//ADDED: 2012-01-19
	pbx_builtin_setvar_helper(transferee, "__FEATURECODE", code);
	//ADDED: 2012-01-19
	...
	//ADDED: 2012-02-16
	int type;
	type = check_builtin_feature(code);
	if (type > 0) {
		char data[8192];
		char *value;
		char *extens[8192];
		...
		int exten_found = 0;
		int x;
		ast_db_get("Neotel", (type == 2) ? "Queues" : "Campaigns", data, sizeof(data));
		value = data;
		if (!ast_strlen_zero(value)) {
			num_extens = ast_app_separate_args(value, ':', extens, sizeof(extens) / sizeof(extens[0]));
			for (x = 0; x < num_extens; x++) {
				if (strcasecmp(xferto, extens[x]))
					continue;
				exten_found = 1;
				break;
			}
			if (!exten_found) {
				ast_log(LOG_WARNING, "Unable to find %s: %s\n", (type == 2) ? "Queue" : "Campaign", xferto);
				ast_stream_and_wait(transferer, "pbx-invalid", "");
				finishup(transferee);
				return AST_FEATURE_RETURN_SUCCESS;
			}
...
static int builtin_atxfer(struct ast_channel *chan, struct ast_channel *peer, struct ast_bridge_config *config, const char *code, int sense, void *data)
{
	...
	//ADDED: 2012-01-19
	pbx_builtin_setvar_helper(transferer, "__FEATURECODE", code);
	pbx_builtin_setvar_helper(transferee, "__FEATURECODE", code);
	//ADDED: 2012-01-19
...
static struct ast_call_feature builtin_features[] = {
	...
	//ADDED: 2012-01-19
	{ AST_FEATURE_REDIRECT, "Blind Transfer 2", "blindxfer2", "#", "#", builtin_blindtransfer, AST_FEATURE_FLAG_NEEDSDTMF, "" },
	{ AST_FEATURE_REDIRECT, "Blind Transfer 3", "blindxfer3", "#", "#", builtin_blindtransfer, AST_FEATURE_FLAG_NEEDSDTMF, "" },
	{ AST_FEATURE_REDIRECT, "Attended Transfer 2", "atxfer2", "", "", builtin_atxfer, AST_FEATURE_FLAG_NEEDSDTMF, "" },
	{ AST_FEATURE_REDIRECT, "Attended Transfer 3", "atxfer3", "", "", builtin_atxfer, AST_FEATURE_FLAG_NEEDSDTMF, "" },
	//ADDED: 2012-01-19
};
...
static int check_builtin_feature(const char *code)
{
	...
	for (x = 0; x < FEATURES_COUNT; x++) {
		if (strcasecmp(builtin_features[x].exten, code))
			continue;
		if (!strcasecmp(builtin_features[x].sname, "blindxfer2"))
			res = 2;
		else if (!strcasecmp(builtin_features[x].sname, "blindxfer3"))
			res = 3;
		break;
	}
...
static int feature_interpret(struct ast_channel *chan, struct ast_channel *peer, struct ast_bridge_config *config, const char *code, int sense) {
	...
	//ORIGINAL: 2014-08-29
	//CHANGED: 2014-08-29
	char dynamic_features_buf[1024];
	//CHANGED: 2014-08-29
