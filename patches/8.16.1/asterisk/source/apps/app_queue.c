	...
	ao2_iterator_destroy(&mem_iter);
 	...
 	if (!found) {
		if ((m = create_queue_member(interface, membername, penalty, paused, state_interface))) {
			...
			int calls = 0, lastcall = 0;
			snprintf(key, sizeof(key), "%s:%s", q->name, m->interface);
			if (!ast_db_get("Queue/Statistics", key, value, sizeof(value))) {
				sscanf(value, "%d:%d", &calls, &lastcall);
				m->calls = calls;
				m->lastcall = lastcall;
			}
			//ADDED: 2016-10-12
	...
	if (*reason == QUEUE_UNKNOWN && q->maxlen && (q->count >= q->maxlen))
		*reason = QUEUE_FULL;
		...
		ast_manager_event(qe->chan, EVENT_FLAG_CALL, "Join",
			...
			//ADDED: 2012-03-12
			pbx_builtin_getvar_helper(qe->chan, "FEATURECODE")
			//ADDED: 2012-03-12
			);
...
static int ring_entry(struct queue_ent *qe, struct callattempt *tmp, int *busies)
{
	...
	if ((res = ast_call(tmp->chan, location, 0))) {
		...
		manager_event(EVENT_FLAG_AGENT, "AgentCalled",
			...
			"Uniqueid: %s\r\n"
			//ADDED: 2012-03-12
			"ChannelCallingFeatureCode: %s\r\n"
			"DestinationChannelFeatureCode: %s\r\n"
			//ADDED: 2012-03-12
			"%s",
			...
			//ADDED: 2012-03-12
			pbx_builtin_getvar_helper(qe->chan, "FEATURECODE"), pbx_builtin_getvar_helper(tmp->chan, "FEATURECODE"),
			//ADDED: 2012-03-12
...
static void record_abandoned(struct queue_ent *qe)
{
	...
	manager_event(EVENT_FLAG_AGENT, "QueueCallerAbandon",
		...
		qe->chan->name,
		//ADDED: 2008-04-07
		qe->chan->uniqueid, qe->pos, qe->opos, (int)(time(NULL) - qe->start));
	...
	if (shared_lastcall) {
		...
		while ((qtmp = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
			...
			if ((mem = ao2_find(qtmp->members, member, OBJ_POINTER))) {
				...
				//ADDED: 2016-10-12
				char key[256], value[256];
				snprintf(key, sizeof(key), "%s:%s", q->name, mem->interface);
				snprintf(value, sizeof(value), "%d:%d", mem->calls, (int)mem->lastcall);
				ast_db_put("Queue/Statistics", key, value);
				//ADDED: 2016-10-12
				ao2_ref(mem, -1);
			}
			ao2_unlock(qtmp);
		...
		//ADDED: 2016-10-12
		char key[256], value[256];
		snprintf(key, sizeof(key), "%s:%s", q->name, member->interface);
		snprintf(value, sizeof(value), "%d:%d", member->calls, (int)member->lastcall);
		ast_db_put("Queue/Statistics", key, value);
		//ADDED: 2016-10-12
		ao2_unlock(q);
	}	
	...
	if (!peer) {
		...
		if (qe->parent->eventwhencalled)
			manager_event(EVENT_FLAG_AGENT, "AgentConnect",
					...
					//CHANGED: 2014-02-14
					(long) time(NULL) - qe->start, qe->chan->name, (long)(orig - to > 0 ? (orig - to) / 1000 : 0),
					//CHANGED: 2014-02-14
					qe->parent->eventwhencalled == QUEUE_EVENT_VARIABLES ? vars2manager(qe->chan, vars, sizeof(vars)) : "");
...
static int add_to_queue(const char *queuename, const char *interface, const char *membername, int penalty, int paused, int dump, const char *state_interface)
{
	...
	if ((old_member = interface_exists(q, interface)) == NULL) {
		if ((new_member = create_queue_member(interface, membername, penalty, paused, state_interface))) {
			...
			char key[256], value[256];
			int calls = 0, lastcall = 0;
			snprintf(key, sizeof(key), "%s:%s", q->name, new_member->interface);
			if (!ast_db_get("Queue/Statistics", key, value, sizeof(value))) {
				sscanf(value, "%d:%d", &calls, &lastcall);
				new_member->calls = calls;
				new_member->lastcall = lastcall;
			}
			//ADDED: 2016-10-12
...
static void reload_single_member(const char *memberdata, struct call_queue *q)
{
	...
	if ((newm = create_queue_member(interface, membername, penalty, cur ? cur->paused : 0, state_interface))) {
		...
		char key[256], value[256];
		int calls = 0, lastcall = 0;
		snprintf(key, sizeof(key), "%s:%s", q->name, newm->interface);
		if (!ast_db_get("Queue/Statistics", key, value, sizeof(value))) {
			sscanf(value, "%d:%d", &calls, &lastcall);
			newm->calls = calls;
			newm->lastcall = lastcall;
		}
		//ADDED: 2016-10-12
...
//ADDED: 2012-07-31
static int manager_queue_reload_members(struct mansession *s, const struct message *m)
{
	struct call_queue *q;
	struct ao2_iterator queue_iter;
	...
	astman_send_ack(s, m, "Queue members reloaded successfully");
	return 0;
}
//ADDED: 2012-07-31
...
static int load_module(void)
{
	...
	//ADDED: 2012-07-31
	res |= ast_manager_register_xml("QueueReloadMembers", 0, manager_queue_reload_members);
	//ADDED: 2012-07-31
