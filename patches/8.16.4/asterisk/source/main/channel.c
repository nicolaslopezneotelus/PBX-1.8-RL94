...
struct ast_channel *__ast_request_and_dial(const char *type, format_t format, const struct ast_channel *requestor, void *data, int timeout, int *outstate, const char *cid_num, const char *cid_name, struct outgoing_helper *oh)
{
	...
	//ADDED: 2012-07-27
	const char *temp_id = pbx_builtin_getvar_helper(chan, "TEMP_ID");
	if (temp_id) {
		manager_event(EVENT_FLAG_CALL, "NewTempId",
		"Channel: %s\r\n"
		"Uniqueid: %s\r\n"
		"TempId: %s\r\n",
		chan->name, chan->uniqueid, temp_id);
	}
	//ADDED: 2012-07-27
	...
	if (ast_call(chan, data, 0)) {	/* ast_call failed... */
		...
		while (timeout && chan->_state != AST_STATE_UP) {
			...
			if (f->frametype == AST_FRAME_CONTROL) {
				...
				case AST_CONTROL_PROGRESS:
				//ADDED: 2015-08-04
					if (oh && oh->connect_on_earlymedia) {
						*outstate = f->subclass.integer;
						timeout = 0;		/* trick to force exit from the while() */
						break;
					}
...
struct ast_channel *ast_request(const char *type, format_t format, const struct ast_channel *requestor, void *data, int *cause)
{
	...
	*cause = AST_CAUSE_NOTDEFINED;
	...
	if (AST_RWLIST_RDLOCK(&backends)) {
		...
		//ADDED: 2012-07-27
		manager_event(EVENT_FLAG_CALL, "ChannelRequestFailed",
		"Type: %s\r\n"
		"Data: %s\r\n"
		"Cause: %s\r\n",
		type, (char *)data, ast_cause2str(*cause));
		//ADDED: 2012-07-27
		return NULL;
	}
...
static void manager_bridge_event(int onoff, int type, struct ast_channel *c0, struct ast_channel *c1)
{
	...
	ast_manager_event_multichan(EVENT_FLAG_CALL, "Bridge", 2, chans,
		...
		"CallerID2: %s\r\n"
		//ADDED: 2012-03-12
		"Channel1FeatureCode: %s\r\n"
		"Channel2FeatureCode: %s\r\n",
		//ADDED: 2012-03-12
		onoff ? "Link" : "Unlink",
		type == 1 ? "core" : "native",
		c0->name, c1->name,
		...
		//ADDED: 2012-03-12
		pbx_builtin_getvar_helper(c0, "FEATURECODE"), pbx_builtin_getvar_helper(c1, "FEATURECODE")
		//ADDED: 2012-03-12
		);
}
