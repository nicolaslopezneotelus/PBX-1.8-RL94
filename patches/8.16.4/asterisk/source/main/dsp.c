...
struct ast_dsp {
	...
	//ADDED: 2015-08-04
	tone_detect_state_t tone_state[5];
	//ADDED: 2015-08-04
};
...
//ADDED: 2015-08-04
void ast_custom_tone_detect_init(struct ast_dsp *dsp, int index, int freq, int duration, int amp)
{
	if (!dsp) {
		return;
	}

	if (ARRAY_LEN(dsp->tone_state) < index + 1) {
		return;
	}
	
	ast_tone_detect_init(&dsp->tone_state[index], freq, duration, amp);
}
//ADDED: 2015-08-04
	...
	while (len) {
		...
		if (dsp->gsamps == dsp->gsamp_size) {
			...
			case PROG_MODE_CR:
				...
				//DELETED: 2015-08-04
				//ADDED: 2015-08-04
				if ((hz[HZ_425] > TONE_MIN_THRESH) && (hz[HZ_425] > TONE_THRESH * dsp->genergy)) {
					newstate = DSP_TONE_STATE_RINGING;
				} else if (dsp->genergy > TONE_MIN_THRESH * TONE_THRESH) {
					newstate = DSP_TONE_STATE_TALKING;
				} else {
			...
			ast_debug(5, "progmode: %d, hz[HZ_425]: %f, genergy: %f, state: %d, count: %d, newstate: %d\n", dsp->progmode, hz[HZ_425] / (TONE_MIN_THRESH * TONE_THRESH), dsp->genergy / (TONE_MIN_THRESH * TONE_THRESH), dsp->tstate, dsp->tcount, newstate);
			if (newstate == dsp->tstate) {
				...
				case DSP_TONE_STATE_TALKING:
					...
					//ADDED: 2015-08-04
					else if (dsp->features & DSP_PROGRESS_EARLY_MEDIA) {
						res = AST_CONTROL_EARLY_MEDIA;
					}
					//ADDED: 2015-08-04
					break;
...
struct ast_frame *ast_dsp_process(struct ast_channel *chan, struct ast_dsp *dsp, struct ast_frame *af)
{
	...
	//ADDED: 2015-08-04
	if ((dsp->features & DSP_FEATURE_TONE_DETECT)) {
		for (x = 0; x < ARRAY_LEN(dsp->tone_state); x++) {
			if (dsp->tone_state[x].freq == 0) {
				continue;
			}
			if (tone_detect(dsp, &dsp->tone_state[x], shortdata, len)) {
				/* Tone was detected */
				memset(&dsp->f, 0, sizeof(dsp->f));
				dsp->f.frametype = AST_FRAME_DTMF;
				dsp->f.subclass.integer = x;
				outf = &dsp->f;
				goto done;
			}
		}
